...
Пот + c1                          -->  Сгущённый Пот (Centrifuge / Pot Distillery)
dust Правильная + Сгущённый + c2  -->  Кр. Пот Додика
Газ Угара + вода                  -->  Разбавленный Пот
Пот + Охладитель                  -->  Скрытый Пот
Скрытый Пот + 2× Нормис-пыль      --[Vibe Stabilizer]-->  Стабилизированный Вайб + Шлак Игнора (SKU-18)
3× Кабельные Обломки              --[Macerator]-->  2× Нормис-пыль + Газ Угара (SKU-18)
Дым от Пыхчения                   --[Pot Distillery]-->  Газ Угара (SKU-18)
...
### 13.7. Крафт машин (Канон SKU-54)

**Shaped (vanilla / верстак)** — **только LV** машины + LV Smoldering Pukan (ручной старт / bootstrap, см. `vanillaCrafting`).

**Assembler (Сборщик)** — **все тиры (LV/MV/HV+)**:
* Для **LV**: Assembler-рецепты опциональны, но более выгодны (автоматизация, быстрая сборка).
* Для **MV и выше**: сборка **СТРОГО в Assembler** (ручные крафты на верстаке удалены для исключения дубликатов в JEI/EMI).

| Тир | Корпус | Схема | Кабель | Способ сборки |
|---|---|---|---|---|
| LV | Пукан LV | Плата Додика | Честная Сталь | Верстак (Bootstrap) или Ассемблер |
| MV | Пукан MV | Людская | Похуит | Строго Ассемблер |
| HV | Пукан HV | Запиздош | Кр. Пот Додика | Строго Ассемблер |
...
### Архитектура и прогрессия ArthurTech (Steam → UHV)
