...
def generate_craft_tree_md():
    content = f"# ArthurTech / SkufAddon — Дерево Крафтов и Прогрессия (Steam → UHV)\n\n"
    content += "Данный документ содержит полное интерактивное дерево крафта всех 10 эр ArthurTech, от первого Скуфитового Слитка до Деревенского Покоя (UHV).\n\n"
    content += "## 📊 Граф прогрессии (Mermaid Flowchart)\n\n"
    content += MERMAID_GRAPH + "\n\n"
    content += RECIPE_TABLE_MD
    
    with open(CRAFT_TREE_MD, "w", encoding="utf-8") as f:
        f.write(content)
    print("Generated:", CRAFT_TREE_MD)

def update_project_logic_md():
    if not os.path.exists(PROJECT_LOGIC_MD):
        return
    with open(PROJECT_LOGIC_MD, "r", encoding="utf-8") as f:
        text = f.read()
    ...
    if marker in text:
        ...
        with open(PROJECT_LOGIC_MD, "w", encoding="utf-8") as f:
            f.write(new_text)
        print("Updated:", PROJECT_LOGIC_MD)

if __name__ == "__main__":
    generate_craft_tree_md()
    update_project_logic_md()
